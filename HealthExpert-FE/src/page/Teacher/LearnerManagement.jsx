...
export default function LearnerManagement() {
  ...
  const fetchLearners = async (courseId) => {
    try {
      ...
      setIsLearnerModalVisible(true); // Open the modal
    } catch (error) {
  ...
  const fetchAccountIdByUserName = async (userName) => {
    try {
      ...
      const accountId = response.data; // Adjust based on your API response structure
      navigate(`/schedule/${accountId}`); // Navigate to ScheduleManagement with the account ID
    } catch (error) {
  ...
  const learnerColumns = [
    {
      ...
      key: "userName",
      render: (text) => (
        <a onClick={() => fetchAccountIdByUserName(text)}>{text}</a> // Make the username clickable
      ),
    },
    {
      title: "Email",
